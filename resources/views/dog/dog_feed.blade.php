      ...
      $.ajax({url: "/notifications", success: function(result){
        ...
        for (var i = 0; i < result.length; i++) {
          switch (result[i].marker) {
            ...
            break;
            case 1:
            $('#notifications').append('<li>'+result[i].title+'<br> <center><a href="/dog/'+result[i].friend_id+'">'+result[i].friend_name+'</a></center></li><li><a class="btn btn-success btn-sm" href="{{ route('friend.acceptFriend', ['id'=>$friend->id]) }}">Aceptar</a><a class="btn btn-danger btn-sm" href="{{ route('friend.acceptFriend', ['id'=>$friend->id]) }}">Rechazar</a></li>');
            break;
            case 2:
