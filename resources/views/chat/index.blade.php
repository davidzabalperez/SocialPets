      ...
      $.ajax({url: "/notifications", success: function(result){
        ...
        for (var i = 0; i < result.length; i++) {
          switch (result[i].marker) {
            case 0:
            $('#notifications').append('<li><a href="{{route('dog.show', $friend->dog->id)}}"><div class="icon-circle bg-red"><i class="material-icons">delete_forever</i></div><div class="menu-info"><h4>'+result[i].title+'</h4><p>'+result[i].user_id+'</p></div></a></li>');
            break;
            case 1:
            $('#notifications').append('<li>'+result[i].title+'<br> <center><a href="/dog/{{$friend->dog->id}}">{{$friend->dog->name}}</a></center></li><li><a class="btn btn-success btn-sm" href="{{ route('friend.acceptFriend', ['id'=>$friend->id]) }}">Aceptar</a><a class="btn btn-danger btn-sm" href="{{ route('friend.acceptFriend', ['id'=>$friend->id]) }}">Rechazar</a></li>');
            break;
            case 2:
